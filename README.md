...
# Download calibration artifact (required for default inference)
...
2. Click on the files you need:
   ...
   - `calibration.json` - Post-hoc calibration artifact (required for default inference)
...
```bash
# CSV file (calibrated probabilities by default)
python inference_file.py -f /path/to/ecg_file.csv -m ./models/ecg_cct_small.pth

# XML file (automatic conversion)
python inference_file.py -f /path/to/ecg_file.xml -m ./models/ecg_cct_small.pth

# Uncalibrated probabilities (opt-out)
python inference_file.py -f /path/to/ecg_file.csv -m ./models/ecg_cct_small.pth --uncalibrated

# Explicit calibration artifact path
python inference_file.py -f /path/to/ecg_file.csv -m ./models/ecg_cct_small.pth --calibration ./models/calibration.json
```
